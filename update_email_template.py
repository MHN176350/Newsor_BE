...
try:
    ...
    template, created = EmailTemplate.objects.get_or_create(
        name='ContactUs_Response',
        defaults={'content': ''}
    )
    
    if created:
        print("Created new email template: ContactUs_Response")
    else:
        print("Updating existing email template: ContactUs_Response")
    ...
    print(f'Error updating email template: {str(e)}')
    sys.exit(1)
